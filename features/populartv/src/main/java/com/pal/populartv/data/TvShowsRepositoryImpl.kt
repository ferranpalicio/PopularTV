...
class TvShowsRepositoryImpl @Inject constructor(
    ...
    override suspend fun getTvShows(): Result<List<TvShow>> {
        ...
        if (System.currentTimeMillis() - appSettings.lastTimeDataSaved() > timeRange) {
            ...
            page++
            return try {
                val roomData: List<com.pal.populartv.data.local.TvShowRoomEntity> = getDataFromNetworkAndSaveIt(page)
            ...
            return if (page == 0) {
                val localData: List<com.pal.populartv.data.local.TvShowRoomEntity> = localDataProvider.requestData(page)
    ...
    private suspend fun getDataFromNetworkAndSaveIt(newPage: Int): List<com.pal.populartv.data.local.TvShowRoomEntity> {
        val networkData: List<com.pal.populartv.data.net.dto.TvShowDto> = networkDataProvider.requestData(newPage)
        val roomData: List<com.pal.populartv.data.local.TvShowRoomEntity> =
            networkData.map {
